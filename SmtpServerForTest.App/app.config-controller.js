...
(function (_) {
    ...
    app.controller('AccountConfigController', function ($scope, $modal, $q, configAPI) {
        ...
        $scope.add = function () {
            return doAccountDialog({}).then(function (newAccount) {
                return $scope.config.Accounts.push(newAccount);
            });
        };

        $scope.edit = function (index) {
            var account = $scope.config.Accounts[index];
            doAccountDialog(JSON.parse(JSON.stringify(account))).then(function (account) {
                return $scope.config.Accounts[index] = account;
            });
        };

        $scope.remove = function (index) {
            return $scope.config.Accounts.splice(index, 1);
        };
    });
    ...
    app.controller('SmtpEndPointConfigController', function ($scope, $modal, $q, configAPI) {
        var doEndPointDialog = function (endPoint, otherEndPoints) {
            ...
            scope.ok = function () {
                var isConflicted = Enumerable.from(otherEndPoints).any(function (ep) {
                    return scope.target.Address == ep.Address && scope.target.Port == ep.Port;
                });
        ...
        $scope.add = function () {
            ...
            doEndPointDialog({}, otherEndPoints).then(function (newEndPoint) {
                return $scope.config.SmtpEndPoints.push(newEndPoint);
            });
        };
        ...
        $scope.edit = function (index) {
            ...
            doEndPointDialog(endPoint, otherEndPoints).then(function (endPoint) {
                return $scope.config.SmtpEndPoints[index] = endPoint;
            });
        };

        $scope.remove = function (index) {
            return $scope.config.SmtpEndPoints.splice(index, 1);
        };
    });
})(_ || (_ = {}));
